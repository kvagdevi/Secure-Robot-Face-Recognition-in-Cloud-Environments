...
def gen_chaos_seq(m,n):
    global x0,y0,z0,a,bb,c,N
    N=m*n*4
    x= np.array((m,n*4))
    y= np.array((m,n*4))
    z= np.array((m,n*4))
    t = np.linspace(0, tmax, N)
    f = odeint(lorenz, (x0, y0, z0), t, args=(a, bb, c))
    x, y, z = f.T
    x=x[:(N)]
    y=y[:(N)]
    z=z[:(N)]
...
def recover_image(b, g, r, iname, savepathh, filename):
    img = cv2.imread(iname)
    img[:, :, 2] = r
    img[:, :, 1] = g
    img[:, :, 0] = b
    outpath = r'C:\Users\OEM\Desktop\chiru\USBDrive\untitled\Work\image-Encryption-dna-encoding-master\images\Train_encrypted'
    ...
    # img.save(r'C:\Users\OEM\Desktop\chiru\USBDrive\untitled\Work\image-Encryption-dna-encoding-master\images\Test_encrypted/' + basename + '.jpg')
    cv2.imwrite(savepathh + basename + '.png', img)
...
for test in string:
    if num == 0:
        ...
        savepath = r'D:\New folder\PCA_FaceRecognition\dna/'
    if num != 0:
    ...
    if len(os.listdir(savepath)) != 100:
        ...
        for folder in folders:
            i = 1
            for imagename in glob.glob(folder + '/*.pgm'):  # assuming jpg
                file_path = imagename
                print(file_path)
                ...
                #file_path = r'C:\Users\OEM\Desktop\chiru\colorferet\colorferet\dvd2\gray_feret_cd1\test\inp\00060fa001d_931230.jpg'
                filename = imagename
                savepathh = file_path


                img = recover_image(b, g, r, imagename, savepathh, filename)
                i = i + 1